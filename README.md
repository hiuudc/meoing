# Meoi Bridge v4 - ChatGPT Web, no API or MCP
...
Use Node.js 22 LTS or newer. This workspace includes a portable Node 22 runtime under `.tools/`.
...
1. Open `chrome://extensions`, enable **Developer mode**, choose **Load unpacked**, and select `dist-extension`.
...
The page and extension use wire protocol v4. A successful coaching result looks like this:
...
`npm run build` creates only the website and unpacked extension output. Existing workspace/library data remains browser-local; the temporary-data rule applies specifically to content returned through Meoi Bridge.

...
function localResult(
  ...
  score: number,
  correction: string,
  errorMessage = "Câu trả lời chưa khớp với đáp án.",
): LocalGradeResult {
  ...
  return {
    ...
    status,
    score: bounded,
    correctParts: status === "correct" ? ["Toàn bộ câu trả lời"] : bounded > 0 ? ["Một phần câu trả lời"] : [],
    ...
    nextHint: status === "correct" ? "Tiếp tục câu tiếp theo." : question.hint ?? "Xem lại phần lý thuyết liên quan rồi thử lại.",
  };
}
...
export function gradeAnswer(question: LessonQuestion, answer: QuestionAnswer): GradeResult {
  switch (question.type) {
    ...
    case "multipleChoice": {
      ...
      return localResult(question, score, correction, "Hãy chọn đủ đáp án đúng và bỏ các lựa chọn thừa.");
    }
    case "trueFalse":
      return localResult(question, typeof answer === "boolean" && answer === question.correct ? 1 : 0, question.correct ? "Đúng" : "Sai");
    case "fillBlank":
    ...
    case "multiCloze": {
      ...
      return localResult(question, correct / question.blanks.length, question.blanks.map((blank) => blank.acceptedAnswers[0]).join(" · "), "Một hoặc nhiều chỗ trống chưa đúng.");
    }
    case "wordBank": {
      ...
      return localResult(question, score, question.correctOrderIds.map((id) => labels.get(id) ?? id).join(" "), "Thứ tự từ chưa hoàn chỉnh.");
    }
    case "matching": {
      const answers = asMap(answer);
      const correct = question.pairs.filter((pair) => answers[pair.leftId] === pair.rightId).length;
      return localResult(question, correct / question.pairs.length, question.pairs.map((pair) => `${pair.left} → ${pair.right}`).join("; "), "Một hoặc nhiều cặp chưa khớp.");
    }
    case "reorderTokens": {
      const score = compareOrdered(asStrings(answer), question.correctOrderIds);
      const labels = new Map(question.tokens.map((token) => [token.id, token.label]));
      return localResult(question, score, question.correctOrderIds.map((id) => labels.get(id) ?? id).join(" "), "Các token chưa ở đúng thứ tự.");
    }
    case "reorderDialogue": {
      const score = compareOrdered(asStrings(answer), question.correctOrderIds);
      const turns = new Map(question.turns.map((turn) => [turn.id, `${turn.speaker}: ${turn.label}`]));
      return localResult(question, score, question.correctOrderIds.map((id) => turns.get(id) ?? id).join(" → "), "Các lượt thoại chưa đúng trình tự.");
    }
    case "categorize": {
      const answers = asMap(answer);
      ...
      return localResult(question, correct / question.items.length, question.items.map((item) => `${item.label} → ${categories.get(item.categoryId) ?? item.categoryId}`).join("; "), "Một hoặc nhiều mục đang ở sai nhóm.");
    }
    case "translation":
      return ai("semantic");
    case "shortAnswer":
